...
def create_index_if_not_exists(client, index_name):
    ...
    # Always recreate to avoid stale mappings
    if client.indices.exists(index=index_name):
...
def ingest_chunks_into_opensearch(client, index_name, chunks):
    ...
    try:
        helpers.bulk(client, actions)
        print(f"Ingested {len(actions)} chunks into '{index_name}'.")
    except Exception as e:
        print(f"Error ingesting into '{index_name}': {e}")
        raise


def ingest_all_content_into_opensearch(process_image, processed_table, semantic_chunks, index_name):
    from helper import get_opensearch_client
    client = get_opensearch_client("localhost", 9200)
    ...
    # Images
    img_chunks = prepare_chunks_for_ingstion(process_image)
    ingest_chunks_into_opensearch(client, index_name, img_chunks)
    ...
    # Text chunks
    text_chunks = prepare_chunks_for_ingstion(semantic_chunks)
...
if __name__ == "__main__":
    ...
    processed_image = process_images_with_captions(raw_chunks, use_gemini=False)

    processed_tables = process_tables_with_descriptions(raw_chunks, use_gemini=False)
    ...
    index_name = "pdf_content"
    ingest_all_content_into_opensearch(processed_image, processed_tables, semantic_chunks, index_name)
